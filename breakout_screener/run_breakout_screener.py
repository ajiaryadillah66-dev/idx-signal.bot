...
from breakout_screener.screener import screen_tickers, group_by_dashboard, format_table, format_bounce_table
...
def print_dashboard(dashboard):
    ...
    for section in ["ENTRY", "SIAGA", "RADAR"]:
        ...
        print(format_table(items, top_n=15) if items else "(kosong)")

    print(f"\n=== BOUNCE ({len(dashboard['BOUNCE'])} saham) ===")
    print(format_bounce_table(dashboard["BOUNCE"], top_n=15) if dashboard["BOUNCE"] else "(kosong)")

    print(f"\n=== OVEREXTENDED ({len(dashboard['OVEREXTENDED'])} saham) ===")
    print(format_table(dashboard["OVEREXTENDED"], top_n=15) if dashboard["OVEREXTENDED"] else "(kosong)")
...
def main():
    ...
    if "--telegram" in sys.argv:
        ...
        bounce_items = dashboard["BOUNCE"][:10]
        if bounce_items:
            lines.append("*🎯 BOUNCE (support-based, filosofi beda dari 3 mode di atas)*")
            for r in bounce_items:
                reasons = "; ".join(r["bounce_info"]["details"])
                lines.append(f"• {r['ticker']} @ {r['price']} - {reasons}")
            lines.append("")
