...
def screen_tickers(tickers, period=None):
    ...
    for t, df in raw.items():
        ...
        mode = classify_mode(df_ind, score_result)
...
def group_by_dashboard(results):
    """Bagian 21 -- kelompokkan hasil ke RADAR / SIAGA / ENTRY / OVEREXTENDED."""
    dashboard = {"RADAR": [], "SIAGA": [], "ENTRY": [], "OVEREXTENDED": []}
    for r in results:
        if r["warnings"]:
...
def format_table(results, top_n=20) -> str:
    """Bagian 15 -- format tabel output ringkas (teks/markdown)."""
    header = (
